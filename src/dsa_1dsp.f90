program dsa_1D_spherical
  ...
  character(len=:), allocatable ::  fname, path
  character(len=:), allocatable ::  file1,file2,file3,file4, &
  name_sl,name_pt,name_dist,Time_bin,Energy_bin
  ...
  integer :: mype,npe,mpierr,seed(630)
  ...
  if(mype.le.9)then
   write(tmp,'(i1)')mype
   fname=trim(file1)//tmp(1:1)//'.dat'
   open(901,file=trim(path)//trim(fname),status='unknown' &
    ,action="READWRITE")
   fname=trim(file2)//tmp(1:1)//'.dat'
   open(902,file=trim(path)//trim(fname),status='unknown' &
    ,action="READWRITE")
   fname=trim(file3)//tmp(1:1)//'.dat'
   open(903,file=trim(path)//trim(fname),status='unknown' &
    ,action="READWRITE")
   fname=trim(file4)//tmp(1:1)//'.dat'
   open(904,file=trim(path)//trim(fname),status='unknown' &
    ,action="READWRITE")
 elseif( mype.ge.10 .and. mype.lt.100)then
   write(tmp,'(i2)')mype
   fname=trim(file1)//tmp(1:2)//'.dat'
   open(901,file=trim(path)//trim(fname),status='unknown' &
    ,action="READWRITE")
   fname=trim(file2)//tmp(1:2)//'.dat'
   open(902,file=trim(path)//trim(fname),status='unknown' &
    ,action="READWRITE")
   fname=trim(file3)//tmp(1:2)//'.dat'
   open(903,file=trim(path)//trim(fname),status='unknown' &
    ,action="READWRITE")
   fname=trim(file4)//tmp(1:2)//'.dat'
   open(904,file=trim(path)//trim(fname),status='unknown' &
    ,action="READWRITE")
 elseif( mype.ge.100 .and. mype.lt.1000)then
  write(tmp,'(i3)')mype
  fname=trim(file1)//tmp(1:3)//'.dat'
  open(901,file=trim(path)//trim(fname),status='unknown' &
    ,action="READWRITE")
  fname=trim(file2)//tmp(1:3)//'.dat'
  open(902,file=trim(path)//trim(fname),status='unknown' &
    ,action="READWRITE")
  fname=trim(file3)//tmp(1:3)//'.dat'
  open(903,file=trim(path)//trim(fname),status='unknown' &
    ,action="READWRITE")
  fname=trim(file4)//tmp(1:3)//'.dat'
  open(904,file=trim(path)//trim(fname),status='unknown' &
    ,action="READWRITE")
else
 write(tmp,'(i4)')mype
 fname=trim(file1)//tmp(1:4)//'.dat'
 open(901,file=trim(path)//trim(fname),status='unknown' &
  ,action="READWRITE")
 fname=trim(file2)//tmp(1:4)//'.dat'
 open(902,file=trim(path)//trim(fname),status='unknown' &
  ,action="READWRITE")
 fname=trim(file3)//tmp(1:4)//'.dat'
 open(903,file=trim(path)//trim(fname),status='unknown' &
  ,action="READWRITE")
 fname=trim(file4)//tmp(1:4)//'.dat'
 open(904,file=trim(path)//trim(fname),status='unknown' &
  ,action="READWRITE")
end if
...
name_sl ='split_levels.dat'
if(mype.eq.0)then
  open(45,file=trim(path)//trim(name_sl),status='unknown')
...
tmin=tmin_data
t=tmin-dt
name_pt ='prms_vs_t.dat'
if(mype.eq.0) then
  open(44,file=trim(path)//trim(name_pt),status='unknown')
end if
do
  t=t+dt
  if(t.gt.tmax) exit
...
if(mype.eq.0) then
  Energy_bin ='E_bin.dat'
  open(801,file=trim(path)//trim(Energy_bin),status='unknown')
  write(801,'(1000e15.6)')E_bin/keV
  close(801)
  Time_bin ='t_bin.dat'
  open(802,file=trim(path)//trim(Time_bin),status='unknown')
...
do n = 1,npart
  ...
  do
    if( isp.eq.0 )then
      ! to get a source that falls as 1/r^2, we pick a time randomly 
        ! between 0 and tmax and assume the particle is released at the shock.  
      ! Because the source flux depends on the ambient density (1/r^2) and 
        ! the bulk flow speed (constant), and the shock surface expands as r^2, 
      ! the approach gives the desired result.
    ...
    do
      ...
      ! advance to the next position and momentum with stochastic integration method
      ...
      if(r.lt.Rmin .or. r.gt.Rmax .or. r_shock.gt.Rshmax) exit
      if(t.gt.tmax) exit
    ...
    if( lev.eq.1 ) exit   ! need to go back and do all split particles
  end do 
  ...
  ! the normalization is such that the density of energetic particles computed
  ! from this spectrum is n_ep (an input). 
  name_dist = 'dist'
  if(mod(n,1).eq.0)then
    if(mype.le.9)then
     write(tmp,'(i1)')mype
     fname=trim(name_dist)//tmp(1:1)//'.dat'
     open(14,file=trim(path)//trim(fname),status='unknown')
   elseif( mype.ge.10 .and. mype.lt.100)then
     write(tmp,'(i2)')mype
     fname=trim(name_dist)//tmp(1:2)//'.dat'
     open(14,file=trim(path)//trim(fname),status='unknown')
   elseif( mype.ge.100 .and. mype.lt.1000)then
     write(tmp,'(i3)')mype
     fname=trim(name_dist)//tmp(1:3)//'.dat'
     open(14,file=trim(path)//trim(fname),status='unknown')
   else
     write(tmp,'(i4)')mype
     fname=trim(name_dist)//tmp(1:4)//'.dat'
     open(14,file=trim(path)//trim(fname),status='unknown')
   end if
