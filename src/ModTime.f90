...
module PT_ModTime
  ...
  ! 1. PTTime and DataInputTime are set to zero in the current file
  ...
  ! 3. If the PT model works within the SWMF in the first
  !    init_session command in the PT_wrapper both PTTime and
  ...
  !    DataInputTime is read from the first MhData file AND PTTime is
  ...
  !    The PTTime keeps unchanged and is LESS THAN DataInputTile.
  ! 2. While reading MhData file a new value is assigned to
  !    DataInputTime which is written in the said file. The PTTime
  !    keeps unchanged and is LESS THAN DataInputTile.
  ! 3. In the course of PT_run the difference in values of density
  !    related to "old" state at PTTime and new one at DataInputTime
  ...
  !    from PTTime to DataInputTime using advance routine. At the
  !    end of advance routine the PTTime is synchronized with
  !    DataInputTime.
  ...
  ! PTTime, we can obtain the date and time, which may be used
